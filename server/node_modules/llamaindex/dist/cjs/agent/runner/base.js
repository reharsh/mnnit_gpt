...
Object.defineProperty(exports, "AgentRunner", {
    enumerable: true,
    get: function() {
        return AgentRunner;
    }
...
const validateStepFromArgs = (taskId, input, step, kwargs)=>{
    if (step) {
        ...
        if (!input) return;
        return new _types.TaskStep(taskId, step, input, kwargs);
    }
};
class AgentRunner extends _types1.BaseAgentRunner {
    agentWorker;
    state;
    memory;
    callbackManager;
   ...
   */ constructor(params){
        ...
        this.state = params.state ?? new _types1.AgentState();
        this.memory = params.memory ?? new _ChatMemoryBuffer.ChatMemoryBuffer({
            chatHistory: params.chatHistory
        });
        this.callbackManager = params.callbackManager ?? new _CallbackManager.CallbackManager();
   ...
   */ createTask(input, kwargs) {
        ...
        const task = new _types.Task({
            taskId: (0, _crypto.randomUUID)(),
            input,
            memory: this.memory,
            extraState,
            ...kwargs
        ...
        const taskState = new _types1.TaskState({
            task,
            stepQueue: [
                initialStep
            ]
   ...
   */ getUpcomingSteps(taskId, kwargs) {
    ...
    async _runStep(taskId, step, mode = _index.ChatResponseMode.WAIT, kwargs) {
        ...
        if (mode === _index.ChatResponseMode.WAIT) {
            curStepOutput = await this.agentWorker.runStep(curStep, task, kwargs);
        } else if (mode === _index.ChatResponseMode.STREAM) {
   ...
   */ async runStep(taskId, input, step, kwargs = {}) {
        ...
        return this._runStep(taskId, curStep, _index.ChatResponseMode.WAIT, kwargs);
    }
    /**
   ...
   */ async streamStep(taskId, input, step, kwargs) {
        const curStep = validateStepFromArgs(taskId, input, step, kwargs);
        return this._runStep(taskId, curStep, _index.ChatResponseMode.STREAM, kwargs);
    }
    /**
   ...
   */ async finalizeResponse(taskId, stepOutput, kwargs) {
        ...
        if (!(stepOutput.output instanceof _index.StreamingAgentChatResponse)) {
            if (!(stepOutput.output instanceof _index.AgentChatResponse)) {
    ...
    async _chat({ message, toolChoice, stream }) {
        ...
        let resultOutput;
        const mode = stream ? _index.ChatResponseMode.STREAM : _index.ChatResponseMode.WAIT;
        while(true){
   ...
   * Resets the agent.
   */ reset() {
        this.state = new _types1.AgentState();
    }
